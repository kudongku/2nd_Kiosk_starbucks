...
public class Order {
    ...
    public void printCategory() { //메인 메뉴판
        ...
        if(inputNum<drink_typeList.size()+1&&inputNum!=0){
            printTypedDrink(inputNum);
        }else {
            runOtherOption(inputNum);
        }
    }

    private void printTypedDrink(int inputNum) {
        System.out.println("-----------------------------------\n" +
        ...
        for (Drink drink : drinkList) {
            ...
            if (drink.drink_type.equals(drink_typeList.get(inputNum - 1))) {
                ...
                System.out.println(type_drinkList.size() + ". " +
                        buildStr(drink.name) + drink.price + "  | " + drink.explanation);
            }
        ...
        if(num2<8){
            selectSize(num2, type_drinkList.get(--num2));
    ...
    private void selectSize(int num2, Drink drink_ordered) {
        System.out.println("""
                        -----------------------------------
                        사이즈를 선택해주세요.
                        1.톨 (기본료 그대로)
        ...
        for(String str: cartList.keySet()){
            ...
            for(Drink drink: drinkList){
                ...
                if(drink.name.equals(str.split(" ")[0])){
                    System.out.println(size_price+"원");
                    total_price += size_price*cartList.get(str);
                }
